...
void LRUK_Replacer::Remove(frame_id_t frame_id) {
  lock_guard<mutex> lock(latch);
...
void LRUK_Replacer::SetEvictable(frame_id_t frame_id, bool set_evictable) {
  lock_guard<mutex> lock(latch);
  if (map.find(frame_id) != map.end()) {
...
void LRUK_Replacer::RecordAccess(frame_id_t frame_id) {
  lock_guard<mutex> lock(latch);
  current_timestamp++;
  if (map.find(frame_id) == map.end()) {
